...
def haymarket_and_leith
  ...
  execute(<<-SQL)
  SELECT DISTINCT
    routesb.company, routesb.num
  FROM
    routes AS routesa
    JOIN
      routes AS routesb ON (routesa.company = routesb.company AND routesa.num = routesb.num)
  WHERE
...
def craiglockhart_to_sighthill
  ...
  JOIN routes start_route ON start_route.stop_id = start_stop.id
  JOIN routes s_to_t_routes ON s_to_t_routes.company = start_route.company AND s_to_t_routes.num = start_route.num
  JOIN stops t_stops ON t_stops.id = s_to_t_routes.stop_id
