...
function flatten(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (!Array.isArray(element)){
            ...
            flattend.push(...flatten(element))

        }
    }
    // data.forEach(element => {
    //     console.log(flattend.push(flatten(element)))
    // });
    return flattend
...
function fileFinder(directories, targetFile) {
    for (let ele in directories) {
        if (fileFinder(directories[ele], targetFile) === true) {
            return true;
        } else if (ele === targetFile) {
            return true
        }
    } return false;
}
...
function pathFinder(directories, targetFile) {
    ...
    //     console.log(element)
    //     let path = pathFinder(element, targetFile ) 
    //     if (path === null) { 
    //         return null
    //     } else if (directories[i] === targetFile) {
    //         console.log(targetFile)
    //     }else {
    //         console.log('/' + directories[i] + path)
    //     }
        
    // }
    for (let ele in directories) {
        if (pathFinder(directories[ele], targetFile) !== null) 
            return ele + pathFinder(directories[ele], targetFile);
         if (ele === targetFile) 
            return  "/" + ele;
